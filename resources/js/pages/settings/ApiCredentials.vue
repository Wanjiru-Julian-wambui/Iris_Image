...
const newSecret = computed(() => props.flash?.new_secret ?? null);

// ── Visibility toggles ──────────────────────────────────────────────
const showNewSecret = ref(false);

const revealList = ref<Record<number, { key: boolean; secret: boolean }>>({});

function isRevealed(id: number, field: 'key' | 'secret'): boolean {
    return revealList.value[id]?.[field] ?? false;
}

function toggle(id: number, field: 'key' | 'secret') {
    if (!revealList.value[id]) revealList.value[id] = { key: false, secret: false };
    revealList.value[id][field] = !revealList.value[id][field];
}
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto max-w-3xl px-4 py-10 space-y-8">
            ...
            <!-- ═══════════════════════════════════════════════════════════════
                 NEW CREDENTIAL REVEAL (only shown immediately after creation)
                 ═══════════════════════════════════════════════════════════════ -->
...
IRIS_API_SECRET={{ showNewSecret ? newSecret : masked(newSecret) }}
            ...
            <div class="rounded-2xl border border-white/8 bg-card overflow-hidden">
                ...
                <ul v-else class="divide-y divide-border">
                    <li
                        ...
                        :key="cred.id"
                        class="px-6 py-4 hover:bg-muted/20 transition-colors space-y-3"
                    >
                        ...
                        <div class="flex items-center gap-3 rounded-xl border border-border bg-background px-4 py-2.5">
                            ...
                            <button
                                @click="toggle(cred.id, 'key')"
                                class="shrink-0 text-[10px] text-muted-foreground hover:text-foreground"
                            >{{ isRevealed(cred.id, 'key') ? 'hide' : 'reveal' }}</button>
                            <button
                                @click="copy(cred.api_key, 'list_key_' + cred.id)"
                                class="shrink-0 text-[10px] text-violet-400 hover:text-violet-300"
                            >{{ copiedId === 'list_key_' + cred.id ? '✓ copied' : 'copy' }}</button>
                        </div>
                        ...
                        <div class="flex items-center gap-3 rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-2.5">
                            ...
                            <button
                                ...
                                class="shrink-0 text-[10px] text-amber-400/80 hover:text-amber-300"
                            >{{ isRevealed(cred.id, 'secret') ? 'hide info' : 'show info' }}</button>
                        </div>
                    </li>
                </ul>
            </div>
