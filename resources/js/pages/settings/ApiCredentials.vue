...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto max-w-3xl px-4 py-10 space-y-8">
            ...
            <div class="flex items-start justify-between gap-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-muted-foreground max-w-lg">
                        ...
                        Authenticate requests with <code class="rounded bg-muted px-1 text-xs text-foreground">X-API-Key</code>
                        and <code class="rounded bg-muted px-1 text-xs text-foreground">X-API-Secret</code> headers.
                    </p>
                </div>
                <button
            ...
            <div v-if="newKey && newSecret" class="rounded-2xl border border-emerald-500/30 bg-emerald-500/8 p-6 space-y-5">
                <div class="flex items-center justify-between">
                    ...
                    >{{ copiedId === 'pair' ? '✓ Copied' : 'Copy both' }}</button>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="flex items-center gap-3 rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3">
                        <code class="flex-1 text-sm font-mono text-amber-300 select-all break-all">{{ newSecret }}</code>
                        <button
...
IRIS_API_SECRET={{ newSecret }}
IRIS_API_URL={{ appUrl }}</pre>
                        <button
                            @click="copy(`IRIS_API_KEY=${newKey}\nIRIS_API_SECRET=${newSecret}\nIRIS_API_URL=${appUrl}`, 'env')"
                            class="absolute top-3 right-3 rounded-lg px-2.5 py-1 text-xs font-semibold transition-colors"
                            :class="copiedId === 'env' ? 'bg-emerald-500/15 text-emerald-400' : 'bg-background text-muted-foreground hover:text-foreground border border-border'"
                    ...
                    <li v-for="cred in credentials" :key="cred.id"
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex flex-wrap items-center gap-2 mt-0.5">
                                <code class="text-xs font-mono text-muted-foreground">{{ cred.api_key }}</code>
                                <button
